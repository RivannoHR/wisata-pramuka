...
<div class="main-content">
    <h2>Reviews for {{ $accommodation->name }}</h2>
    <p>Manage guest reviews and feedback</p>
    
    <a href="{{ route('admin.accommodations') }}" style="display: inline-block; margin-bottom: 20px; padding: 8px 16px; background: #6c757d; color: white; text-decoration: none; border-radius: 4px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 20px; background: white; border: 1px solid #dee2e6;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="padding: 12px; border: 1px solid #dee2e6;">
                        <form method="POST" action="{{ route('admin.accommodations.reviews.destroy', $review->id) }}" style="display: inline-block;" onsubmit="return confirm('Are you sure you want to delete this review?')">
                            @csrf
                            @method('DELETE')
    ...
    @else
        <div style="text-align: center; padding: 40px; background: white; border: 1px solid #dee2e6; margin-top: 20px;">
            <p style="color: #6c757d; font-size: 18px;">No reviews found for this accommodation.</p>
        </div>
    @endif
</div>
@endsection
