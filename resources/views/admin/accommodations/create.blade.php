...
<div style="padding:20px; max-width:100%; display:flex; flex-direction:column; gap:20px">
    ...
    <form action="{{ route('admin.accommodations.roomtypes', $accommodation->id) }}" method="GET" style="margin: 0;">
        <button type="submit" class="submit-button">
            Edit Rooms
        </button>
    </form>
...
@else
<form method="POST" action="{{ route('admin.accommodations.store') }}" class="create-product-form" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group">
        <label>Initial Room Type:</label>
        <input type="text" name="room_name" placeholder="Room Name" value="{{ old('room_name') }}">
        <input type="file" name="room_image">
        <input type="number" name="room_price" placeholder="Price" value="{{ old('room_price') }}" step="0.01">
        @if ($errors->has('room_name') || $errors->has('room_image') || $errors->has('room_price'))
        <span class="error-message">Accommodation must have an initial room type.</span>
        @endif
    </div>

    <input type="hidden" name="facilities" id="facilities-input">
    <button type="submit" class="submit-button">Create Accommodation</button>
</form>
@endif
