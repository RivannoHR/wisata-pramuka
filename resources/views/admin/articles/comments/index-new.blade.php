                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        @if($status !== 'approved')
                                            <button type="button" class="btn btn-sm btn-success" onclick="updateCommentStatus({{ $comment->id }}, 'approved')" title="Approve">
                                                <i class="fas fa-check"></i>
                                            </button>
                                        @endif
                                        @if($status !== 'spam')
                                            <button type="button" class="btn btn-sm btn-warning" onclick="updateCommentStatus({{ $comment->id }}, 'spam')" title="Mark as Spam">
                                                <i class="fas fa-flag"></i>
                                            </button>
                                        @endif
                                        <button type="button" class="btn btn-sm btn-info" onclick="replyToComment({{ $comment->id }})" title="Reply">
...
function bulkApprove() {
    ...
    if (confirm(`Are you sure you want to approve ${ids.length} comment(s)?`)) {
...
function bulkMarkSpam() {
    ...
    if (confirm(`Are you sure you want to mark ${ids.length} comment(s) as spam?`)) {
        bulkUpdateStatus(ids, 'spam');
    }
}

function bulkUpdateStatus(ids, status) {
    // Create and submit bulk update form
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = `/admin/articles/{{ $article->id }}/comments/bulk-update-status`;
...
function replyToComment(id) {
    // This could open a modal or navigate to a reply form
    alert('Reply functionality can be implemented here');
}
